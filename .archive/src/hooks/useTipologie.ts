...
export function useTipologie() {
  ...
  const loadTipologie = async () => {
    ...
    try {
      ...
      const userId = getUserId(); // User ID fisso per app senza autenticazione
  ...
  useEffect(() => {
    ...
    console.log('⚠️ Tipologie hook disabilitato temporaneamente');
    setLoading(false);
  }, []);
  ...
  return { 
    ...
    removeTipologia, 
    updateTipologia,
    refreshTipologie: loadTipologie 
  };
}