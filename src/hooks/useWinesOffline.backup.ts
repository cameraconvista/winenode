...
export const useWines = (): UseWinesOfflineReturn => {
  ...
  const onSyncComplete = useCallback((successCount: number, errorCount: number) => {
    ...
    if (successCount > 0) {
      setTimeout(async () => {
        ...
        setInternalWines(originalHook.wines);
        setInternalSuppliers(originalHook.suppliers);
      ...
      if (isOnline) {
        try {
          ...
          if (originalHook.wines.length > 0) {
            ...
            // FIX CRITICO: Sincronizza state interno con dati freschi
            setInternalWines(originalHook.wines);
            setInternalSuppliers(originalHook.suppliers);
  ...
  return {
    ...
    wines: (isUsingCache || internalWines.length > 0) ? internalWines : originalHook.wines,
    suppliers: (isUsingCache || internalSuppliers.length > 0) ? internalSuppliers : originalHook.suppliers,
